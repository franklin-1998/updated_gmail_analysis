...
def api_all12():
    ...
    while(True):
        ...
        #     print(parse(str(current_DateTime)) > receivedMessages_new['TimeDate'][redundancy_checking])
        #     # print(datetime.datetime.strptime(parse(str(current_DateTime)).strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S").time())
        # sleep(5)
        receivedMessages_new.to_csv("body.csv")
        log_out(access_token)
        break
            # if (current_DateTime - receivedMessages_new['TimeDate'][redundancy_checking]) > 0:
...
def outlook_main():
    ...
    current_DateTime = ast.literal_eval((request.data).decode("utf-8"))
